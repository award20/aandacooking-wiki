---
title: Soybean & Tofu Foods
description: Soybean processing, tofu, Soy Sauce, fermented ingredients, and finished Soybean dishes in A & A Cooking.
---

Soybean is one of A & A Cooking's broadest ingredient chains. It supports cooked ingredients, drinks, tofu, fermentation, sauces, stir fries, glazes, noodles, fried Rice, and Rice bowls.

This page focuses on the food side of that chain. Crop and ingredient foundations remain on [Rice & Soybean](/ingredients/rice-soybean/), while fermentation mechanics are documented under [Fermentation & Preservation](/recipes/fermentation-preservation/) and the [Fermentation Crock](/stations/fermentation-crock/).

## Early Soybean processing

Confirmed processed Soybean items include:

| ID | Display name | Role |
|---|---|---|
| `soybean_roasted` | Roasted Soybean | Cooked Soybean food |
| `soybean_puree` | Soybean Puree | Processing intermediate |
| `soybean_milk` | Soy Milk | Drink and production ingredient |
| `soybean_milk_sweet` | Sweet Soy Milk | Sweetened drink |

Soybean Puree and Soy Milk demonstrate how the crop can move through processing stations before becoming either a finished consumable or a prerequisite for another recipe.

## Tofu

The Soybean chain includes:

| Item ID | Display name | Role |
|---|---|---|
| `soybean_tofu_fresh` | [Fresh Tofu](/recipes/details/fresh-tofu/) | Tofu intermediate |
| `soybean_tofu_seared` | Seared Tofu | Finished tofu side |

[Fresh Tofu](/recipes/details/fresh-tofu/) is made from Soy Milk and Apple Vinegar in the [Saucepan](/cooking/saucepan/). Tofu then feeds into later dishes, including `soybean_tofu_stir_fry_pickled`.

## Soybean stir fries

The established stir fry branch includes:

| ID | Display name |
|---|---|
| `soybean_stir_fry_mushroom` | Mushroom Soybean Stir Fry |
| `soybean_stir_fry_chicken` | Chicken Soybean Stir Fry |
| `soybean_tofu_stir_fry_pickled` | Pickled Tofu Stir Fry |

These recipes connect Soybean and tofu with the direct cooking systems covered under [Stir Fries & Seared Foods](/recipes/stir-fries-seared-foods/) and the [Wok](/cooking/wok/).

## Fermented Soybean chain

The established fermentation branch includes:

- Fermented Soybeans
- Soy Sauce Mash
- Soy Sauce
- `soybean_sauce_sweet` (Sweet Soy Glaze)

The first three outputs are established parts of the Soybean chain, but their exact item IDs are not published on this page yet. Sweet Soy Glaze is registered as `soybean_sauce_sweet`.

The fermentation branch is important because its outputs become prerequisites for multiple recipe families instead of remaining isolated preserved foods.

## Soy glazed foods

The Sweet Soy Glaze branch includes two glazed protein dishes:

| ID | Display name |
|---|---|
| `chicken_glazed_soy_sauce` | Soy Glazed Chicken |
| `salmon_glazed_soy_sauce` | Soy Glazed Salmon |

These recipes depend on the Soy Sauce branch while belonging to the broader cooked protein catalog.

## Soy Sauce across other recipe families

Soybean processing also feeds into several pages elsewhere in the catalog:

- `rice_fried_chicken_soy_sauce` (Chicken Soy Fried Rice)
- `rice_fried_beef_soy_sauce` (Beef Soy Fried Rice)
- `wheat_noodles_stir_fry_chicken_soy_sauce` (Chicken Soy Noodle Stir Fry)
- `wheat_noodles_stir_fry_beef_soy_sauce` (Beef Soy Noodle Stir Fry)
- `rice_bowl_chicken_soy_sauce` (Soy Chicken Rice Bowl)
- `rice_bowl_salmon_soy_sauce` (Soy Salmon Rice Bowl)

See [Rice Dishes & Bowls](/recipes/rice-dishes-bowls/) and [Pasta & Noodles](/recipes/pasta-noodles/) for those finished meal groups.

## Recipe Book behavior

The Soybean chain is a strong example of [Recipe Book](/recipe-book/overview/) prerequisites. A final dish can depend on an earlier Soybean preparation, a fermented product, cooked Rice or noodles, and another prepared ingredient. The preparation planner can represent those dependencies instead of treating the final dish as a single isolated recipe.

## Recipe detail status

This page covers the established Soybean food families and published item IDs. Exact quantities, fermentation times, cooking temperatures, action counts, and a few remaining item IDs will be added as those recipes receive dedicated documentation.

## Related pages

- [Rice & Soybean](/ingredients/rice-soybean/)
- [Rice Dishes & Bowls](/recipes/rice-dishes-bowls/)
- [Fermentation & Preservation](/recipes/fermentation-preservation/)
- [Fermentation Crock](/stations/fermentation-crock/)
- [Stir Fries & Seared Foods](/recipes/stir-fries-seared-foods/)
- [Pasta & Noodles](/recipes/pasta-noodles/)
- [Drinks](/recipes/drinks/)
- [Recipe Catalog](/recipes/overview/)
